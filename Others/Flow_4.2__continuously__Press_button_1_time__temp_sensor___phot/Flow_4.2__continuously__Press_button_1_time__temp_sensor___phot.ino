...
void loop() {
  ...
  if(buttonstatus == HIGH){                                     //If button is not pressed at all, surrounding temperature is not read and nothing will happen.
                                                                
  do{                                                           //If button is pressed, do this loop (below algorithm)
  photovalue = analogRead(photosensor);                         //read the original value of the photosensor and store it as photovalue
  
  vout = analogRead(tempsensor);                                //read the original value of the temperature sensor and stores it as vout
  ...
  while(a == 0);
    ...
    // do - while loop: will do the statement block (the algorithm - until the last else loop) indefinitely while the condition (a == 0) is met 
    //                  (as long as the condition (a == 0) is met)
    ...
    // System runs continuously when button is pressed. Never stops.

    // Press button 1 time, temp sensor & photo sensor reads surrounding continuously, and displays reading on serial monitor continuously without stopping
