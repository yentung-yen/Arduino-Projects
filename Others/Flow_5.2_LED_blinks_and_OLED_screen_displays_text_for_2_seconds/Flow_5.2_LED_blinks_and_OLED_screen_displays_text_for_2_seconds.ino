...
void loop() {
  ...
  if(buttonstatus == HIGH){  
    ...
    display.println("LED is blinking");            //and prints this at the same time the LED is ON
    ...
    display.setTextSize(1);                        //(text size)
    display.setCursor(0,0);                        //(starting coordinate for the text)
    display.println("LED is not blinking");        //and prints this at the same time the LED is OFF
    display.display();                             //(to tell the arduino to tell the OLED screen to display what we have written)
    delay(500);                                   
}
}

   // LED blinks and OLED screen displays text for 2 seconds continuously when button is pressed
   // Then OFF's when button is released
